...
### 1️⃣4️⃣ 修改位置/布局前必须先截 Figma 截图,改完立刻截 HTML 对比,不允许跳过

**反例**:用户说"两个 oval 位置不对" → 我凭理论分析猜了个修法(加 `height:71px` + `justify-end`) → 直接改代码 → 没截图验证 → 再次报错 → 才开始截图对比。来回三轮才收敛。

**铁律**:任何涉及位置、间距、对齐的修改,强制走这个顺序:
1. `get_screenshot` 拿 Figma 节点截图(先存下来)
2. 修改代码
3. Chrome headless 截 HTML 同区域
4. 两图并排目视确认一致
5. 确认后才给用户看

**不允许"改完直接给用户看"**。用户不应该充当你的截图对比工具。

### 1️⃣5️⃣ 绝对不能盲信 context summary 里的"已分析结论" — 视觉问题必须自己重新验证

**反例**:上一轮 context summary 写道"修法是加 `height:71px` + `justify-content:flex-end`"，我直接套用，没有独立截图验证。结果那个结论本身就是错的。

**铁律**:context summary 对于"谁在哪""做了什么" 基本可信，但对于**"CSS 应该怎么写才能还原 Figma"** 这类视觉判断，**一律不可信**。每次修复视觉问题都必须重走铁律 14 的截图流程，不能把上轮分析当现成答案。

### 1️⃣6️⃣ 不用理论推导 CSS flex 行为 — 截图量像素才是可信来源

**反例**:花大量时间推算 `justify-end`/`justify-center` 在不同 height 下的渲染结果，推了四五轮不同方案，结果全错。正确答案就是"截图一眼看出来"的。

**铁律**:CSS 弹性布局行为反直觉，理论推导出错率极高。遇到对齐/居中/间距问题：
- ❌ 推导"这个 justify-center 在 height=71px 下应该是..."
- ✅ 截 Figma 图 → 截 HTML 图 → 量像素 → 对数值

理论用来理解原理，截图量像素用来做决策。**两者冲突时以截图为准**。
